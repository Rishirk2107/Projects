...
scapps=['FACEBOOK','YOUTUBE','WHATSAPP','INSTAGRAM','TWITTER','TIKTOK','TELEGRAM','SNAPCHAT','PINTEREST']
pl = ['JAVA','CSS', 'RUBY', 'HTML','KOTLIN','PERL','SWIFT','DBMS','PEGGA','JAVASCRIPT','TYPESCRPT','CPP','PHP']
word = random.choice(pl)
guessed = []
abc=word

# Display the initial image
image = pygame.image.load("img_2.png")
images.append(image)

# setting the font style
fo="comicsansms"
font = pygame.font.SysFont(fo, 30)
game_font = pygame.font.SysFont(fo, 80)
letter_font = pygame.font.SysFont(fo, 60)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for i in range(5):
                if hangman_status == 1:
                    image = pygame.image.load("img_3.png")
                    images.append(image)
                elif hangman_status == 2:
                    image = pygame.image.load("img_4.png")
                    images.append(image)
                elif hangman_status == 3:
                    image = pygame.image.load("img_5.png")
                    images.append(image)
                elif hangman_status == 4:
                    image = pygame.image.load("img_6.png")
                    images.append(image)
                elif hangman_status == 5:
                    image = pygame.image.load("img_7.png")
                    images.append(image)
                elif hangman_status == 6:
                    running = False
    screen.blit(image, (150, 150))
    for box in boxes:
        pygame.draw.rect(screen,(95,158,160), box, 2)
    ...
    if game_over:
        ...
        if won==True:
            text = game_font.render(game_text, True, (0, 128, 0))
        else:
            text = game_font.render(game_text, True, (255,0, 0))
